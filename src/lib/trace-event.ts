export async function traceEvent(
  ...
  if (!url || !apiKey) {
    console.error("[journalists-service] RUNS_SERVICE_URL or RUNS_SERVICE_API_KEY not set, skipping trace event");
    return;
  }
  try {
    ...
    console.error("[journalists-service] Failed to trace event:", err);
  }
}
